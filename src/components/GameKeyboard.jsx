    ...
    <div className="mt-9 flex flex-wrap justify-center gap-1">
      {alphabetArray.map((char) => (
        <button
          ...
          key={char}
          className={`w-[calc(100%/9-5px)] rounded-md border bg-purple-400 py-1.5 font-semibold text-white hover:bg-purple-600 ${clickedKeys.includes(char) && "pointer-events-none opacity-40"}`}
        >
          {char.toUpperCase()}
        </button>
      ))}
